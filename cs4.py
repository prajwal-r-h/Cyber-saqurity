'''4) Write a python program that reads a file containing a list of passwords, one per line.
It check search password to see if it meets certain requirements (e.g. at least 8
characters contains both upper case and lower case letters, and at least one number
and one special character). Passwords that satisfy the requirements should be printed
by the program.'''
import re
...
with open('passwords.txt') as f:
    # Read each password from the file and check if it meets requirements
    for password in f:
